...
// -------------------- TICKET CARD COMPONENT (Massive & Data-Rich) --------------------
function TicketCard({ t, onTicketUpdated, role }) {
  ...
  // NOTE: formatDateTime is removed because we now use t.created_at_display from Serializer
  ...
  return (
    ...
    ${t.status === 'RESOLVED' ? 'bg-slate-50/50' : ''}`}>
      ...
      <div className="p-6 md:p-10 space-y-6">
        ...
        <div className="flex justify-between items-start mb-4 border-b border-slate-100 pb-5">
          <div className="space-y-2">
            ...
            <span className="flex items-center gap-3 text-xs font-black uppercase tracking-[0.1em] text-slate-700">
              ...
              {/* FIXED: Using the vendor_name field directly from Backend Serializer */}
              {t.vendor_name}
            </span>
          </div>
        ...
        {/* Timestamps Section - Using Pre-formatted strings from Backend */}
    ...
    <div className="min-h-screen bg-[#FDFDFF] font-sans text-slate-900 flex flex-col">
                ...
                {(role === 'VENDOR' || role === 'ADMIN') && (
                  <div className="lg:col-span-5 xl:col-span-5 space-y-12">
                    <div className="lg:sticky lg:top-36 space-y-12">
                      ...
                      {role === 'ADMIN' && (
                        <div className="bg-white p-10 rounded-[3rem] border-2 border-slate-100 shadow-2xl shadow-slate-100 transition-all hover:border-red-400">
                          <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-10 flex items-center justify-between">Workload Share <Activity size={24} className="text-red-500"/></h3>
                          <img src={`${API_BASE}/static/reports/admin_pie.png?v=${Date.now()}`} className="w-full rounded-[2.5rem] transition-transform hover:scale-[1.03] duration-500" alt="Global Pie" />
                        </div>
                      )}
                    </div>
                  </div>
      ...
      <footer className="bg-white border-t-2 border-slate-100 mt-auto py-20 px-12 w-full">
        <div className="max-w-[1800px] mx-auto flex flex-col lg:flex-row justify-between items-center gap-16">
          ...
          <div className="flex gap-20 text-[11px] font-black uppercase text-slate-400">
            <div className="text-right flex flex-col items-center lg:items-end gap-2"><span>Master Status</span><span className="text-emerald-500 flex items-center gap-2 font-black tracking-widest">● LIVE SYSTEM</span></div>
            <div className="text-right flex flex-col items-center lg:items-end gap-2 font-black"><span>PostgreSQL</span><span className="text-blue-600 tracking-widest italic font-black">v16 Engine</span></div>
          </div>
        </div>
      </footer>
    </div>
